      ...
      {SETUP_PRESETS.map((preset) => {
        ...
        return (
          <button
            ...
            className={cn(
              'relative p-4 rounded-xl border text-left transition-all duration-200 cursor-pointer overflow-hidden flex flex-col gap-3 select-none',
              layout === 'stack' && 'sm:flex-row sm:items-center sm:gap-4',
              isActive
                ? cn(accent.border, accent.bg, 'shadow-lg', accent.glow)
